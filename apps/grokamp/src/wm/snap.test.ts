...
import { clampToViewport, quantizeSize, snapPosition } from "./snap";
...
describe("snapPosition", () => {
  ...
  test("prefers the closest candidate", () => {
    ...
    // moving window at x=400 w=10: right edge 410 → b.left is 5 away; a.right is 5 away from left
    // both distance 5; the earlier candidate wins ties, either is a legal snap
    const pos = snapPosition({ x: 402, y: 100, w: 10, h: 100 }, [a, b], VIEWPORT);
    expect([395, 405]).toContain(pos.x);
  });
});
